...
def drop_index(cursor):
    try:
        cursor.execute("DROP INDEX IF EXISTS idx_data ON benchmark_table")
        return True
    except mysql.connector.Error as err:
...
def calculate_metrics(start_time, end_time, total_operations):
    total_time = end_time - start_time
    ops_per_sec = total_operations / total_time
    return total_time, ops_per_sec
...
def run_benchmark():
    ...
    if connection:
        ...
        total_time, ops_per_sec = calculate_metrics(start_time, end_time, total_operations)
